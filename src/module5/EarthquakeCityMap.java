...
public class EarthquakeCityMap extends PApplet {
	...
	// Make sure you do not select two markers (set lastSelected == null).
	...
	public void mouseClicked() {
		// clear the last selection
		// if a marker was previously selected
		if (lastClicked != null) {
			...
			if (lastClicked instanceof EarthquakeMarker) {
				...
				for (Marker marker: cityMarkers) {
					double distance = earthquake.getDistanceTo(marker.getLocation());
					if (distance > earthquake.threatCircle()) {
				...
				for (Marker marker: quakeMarkers) {
					...
					double distance = earthquake.getDistanceTo(city.getLocation());
					if (distance > earthquake.threatCircle()) {
	...
	/**
	 * Set the earthquake or city marker to be clicked
	 * if the mouse detected within bounds of marker.
	 * @param markers
	 */
	private void selectMarkerIfClick(List<Marker> markers) {
		for (Marker m: markers) {
			if (m.isInside(map, mouseX, mouseY) && lastClicked == null) {
				...
				return; // break from loop if marker found
			}
		}
	}
	
	/**
	 * Hide all other earthquake or city markers aside from the marker 
	 * that was clicked.
	 * @param markers: earthquake or city markers list
	 */
